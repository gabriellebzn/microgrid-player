...
class Player:
	...
	def take_decision(self, time):
		...
		for t in range(time, 48):
			...
			if t == 48:
				lp += self.l_bat[t] == 48, "cnt_batt_" + str(t)
			else:
				...
				lp += self.a[t] == self.a[t - 1] + (
							self.rho_c * np.maximum(self.l_bat[t], 0) - (1 / self.rho_d) * np.minimum(self.l_bat[t],
																									  0)) * self.delta_t, "cnt_batt_" + str(
					t)
...
if __name__ == "__main__":
	myplayer = Player()
	myload = myplayer.compute_load(2)
